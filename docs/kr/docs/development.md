...
이 가이드는 데스크톱 애플리케이션, Android APK, 문서 빌드를 포함한 Dure-Sijang 개발 환경 설정에 대해 다룹니다.
...
Dure-Sijang를 빌드하려면 Rust가 필요합니다. rustup을 사용하여 설치하세요:
...
디버그 바이너리는 `target/debug/dure-sijang` (Windows에서는 `dure-sijang.exe`)에 있습니다.
...
릴리스 바이너리는 `target/release/dure-sijang`에 있습니다.
...
Dure-Sijang는 여러 타겟에 대한 빌드를 지원합니다:
