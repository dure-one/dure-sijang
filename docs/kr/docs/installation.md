...
- 가상 머신에서 실행하는 경우
- Dure-Sijang 실행 시 그래픽 관련 오류가 발생하는 경우
...
Dure-Sijang는 모바일 기기에서 mycart 스토어를 탐색할 수 있는 네이티브 Android APK로 제공됩니다.

### 설치

다음 소스 중 하나에서 APK를 다운로드하여 설치하세요:
...
1. **Dure-Sijang** 앱을 엽니다
2. 메시지가 표시되면 저장소 권한을 부여합니다 (제품 이미지 캐싱용)
3. 앱이 dure.one에서 스토어 디렉토리를 동기화합니다
4. 디렉토리에서 사용 가능한 mycart 스토어를 탐색합니다

### 앱 사용

**스토어 디렉토리:**
- 사용 가능한 mycart 스토어 목록 보기
- 스토어를 탭하여 새 탭에서 열기
- 스토어는 dure.one에서 자동으로 동기화됩니다

**브라우징 모드:**
- **웹뷰 모드**: 모든 기능을 갖춘 전체 웹사이트
- **API 모드**: 네이티브 제품 그리드 및 장바구니 (더 빠르고 데이터 사용량 적음)
- 모드 전환 버튼을 사용하여 모드 전환
...
**북마크:**
- 북마크 아이콘을 탭하여 현재 페이지 저장
- 메뉴에서 북마크에 접근
- 북마크는 앱 재시작 후에도 유지됩니다

### 저장 위치

- **데이터베이스**: `/data/data/app.dure.sijang/dbs/`
- **설정**: `/data/data/app.dure.sijang/files/settings.txt`
- **이미지 캐시**: 내부 캐시 디렉토리 (자동 관리됨)

### 문제 해결

**앱이 시작 시 충돌:**
- Android 설정에서 앱 데이터 지우기
- 앱 재설치
- 사용 가능한 저장 공간 확인
...
- 인터넷 연결 확인
- 저장소 권한 부여
- Android 설정에서 앱 캐시 지우기

**스토어 디렉토리가 동기화되지 않음:**
- 인터넷 연결 확인
- dure.one 접근 가능 여부 확인
- 디렉토리 보기에서 당겨서 새로고침 시도
...
Dure-Sijang에서 그래픽 오류나 빈 창이 표시되는 경우:
1. GPU 드라이버를 업데이트합니다
