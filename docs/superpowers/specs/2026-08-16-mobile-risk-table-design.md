...
- Route enable/disable/uninstall/info-click through `ui.data_mut`/`ctx.data_mut` temp-key
  ...
  `info_clicked_package`) that are polled elsewhere in `dure_sijang_app.rs`.
...
`dure_sijang_app.rs`'s dashboard-counter click handlers for `("stalkerware", 0|1)` and
...
`dure_sijang_app.rs` (~lines 1700-1765) already performs direct `adb::enable_app` /
...
comment at `dure_sijang_app.rs:1766`), so desktop IzzyRisk/Stalkerware uninstall is silently
...
`DureSijangApp` directly — it only matters while a Stalkerware/IzzyRisk drill-down is open,
...
3. **`dure_sijang_app.rs`**: the 6 dashboard-counter click handlers for `"stalkerware"` /
