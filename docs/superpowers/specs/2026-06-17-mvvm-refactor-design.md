...
**Goal:** Refactor Dure-Sijang to use actor-based MVVM architecture with smol async runtime

## Executive Summary

Refactor Dure-Sijang from direct UI-blocking I/O to a clean **actor-based MVVM architecture** where:
...
4. ✅ Add ViewModel to `DureSijangApp`:
   ```rust
...
5. ✅ Call `viewmodel.poll_events(ctx)` in `DureSijangApp::update()`
...
4. ✅ Migrate texture loading:
   - Remove renderer state machines from `dure_sijang_app.rs`
...
2. ✅ Remove old threading code:
   ...
   - Remove `package_loading_thread` from `DureSijangApp`
...
│   ├── dure_sijang_app.rs      # DureSijangApp owns ViewModel
