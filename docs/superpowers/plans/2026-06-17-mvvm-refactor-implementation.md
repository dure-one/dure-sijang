...
**Goal:** Refactor Dure-Sijang to actor-based MVVM architecture with smol async runtime for non-blocking UI operations
...
mobile/src/dure_sijang_app.rs    # Add ViewModel field, poll events
...
/// ViewModel struct - owned by DureSijangApp, coordinates actor communication
...
impl ViewModel {
    ...
    /// Poll for events and update state. Call this in DureSijangApp::update()
...
### Task 3: Integrate ViewModel into DureSijangApp

**Files:**
- Modify: `mobile/src/dure_sijang_app.rs:1-50` (add imports)
- Modify: `mobile/src/dure_sijang_app.rs:224-370` (add field to struct)
- Modify: `mobile/src/dure_sijang_app.rs` (find update method and add poll)

**Interfaces:**
- Consumes: `ViewModel::new()`, `ViewModel::poll_events()` from Task 2
- Produces: `DureSijangApp` with `viewmodel` field accessible in tabs

- [ ] **Step 1: Add viewmodel import to dure_sijang_app.rs**
...
Find the `pub struct DureSijangApp {` definition (around line 224) and add this field before the last field:
```rust
...
Find `impl Default for DureSijangApp` and add to the struct construction:
```rust
...
git commit -m "feat: integrate ViewModel into DureSijangApp

Add ViewModel field to DureSijangApp struct.
...
use crate::dure_sijang_app::UadNgLists;
...
pub struct ViewModelState {
    ...
    pub uad_ng_lists: Option<crate::dure_sijang_app::UadNgLists>,
...
```rust
    ...
    pub fn uad_ng_lists(&self) -> Option<&crate::dure_sijang_app::UadNgLists> {
...
Run: `grep -r "std::thread::spawn\|Arc<Mutex" mobile/src/tab*.rs mobile/src/dure_sijang_app.rs`
...
```bash
git add mobile/src/tab*.rs mobile/src/dure_sijang_app.rs
