...
`dure_sijang_app.rs`). Making the debloat copy `pub(crate)` lets the new module reuse it
...
Expected: no errors from `dlg_mobile_list.rs` itself (errors from `dure_sijang_app.rs`'s
...
### Task 7: Wire `dure_sijang_app.rs` and revive the uninstall handler

**Files:**
- Modify: `mobile/src/dure_sijang_app.rs` (import block, 6 dashboard-counter handler arms, the `dlg_mobile_list.show()` call site, and the dead uninstall handler)
...
In `mobile/src/dure_sijang_app.rs`, next to the existing `use crate::dlg_dashcounter_details::DlgDashCounterDetails;` (line 29), add:
...
At `dure_sijang_app.rs:1108-1111` (`("stalkerware", 0)` arm), change:
...
At `dure_sijang_app.rs:1163-1166` (`("stalkerware", 1)` arm), change:
...
At `dure_sijang_app.rs:1220-1222` (`("izzyrisk", 0)` arm), change:
...
At `dure_sijang_app.rs:1274-1277` (`("izzyrisk", 1)` arm), change:
...
At `dure_sijang_app.rs:1329-1332` (`("izzyrisk", 2)` arm), change:
...
At `dure_sijang_app.rs:1384-1386` (`("izzyrisk", 3)` arm), change:
...
At `dure_sijang_app.rs:1628-1643`, change:
...
At `dure_sijang_app.rs:1768`, the line `// REMOVED: Uninstall confirmation dialog
