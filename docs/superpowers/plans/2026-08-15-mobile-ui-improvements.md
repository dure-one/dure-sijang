...
If all flags show `false`, check `dure_sijang_app.rs` where mobile_list_dialog.show() is called.

Find the call site and verify it's passing the correct boolean values:

```bash
grep -n "mobile_list_dialog.show" mobile/src/dure_sijang_app.rs
```
...
```bash
git add mobile/src/dlg_mobile_list.rs mobile/src/dure_sijang_app.rs
