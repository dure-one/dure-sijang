...
This guide covers setting up a development environment for Dure-Sijang, including building the desktop application, Android APK, and documentation.
...
Rust is required to build Dure-Sijang. Install it using rustup:
...
```bash
git clone https://github.com/dure-one/dure-sijang
cd dure-sijang
```
...
The debug binary will be at `target/debug/dure-sijang` (or `dure-sijang.exe` on Windows).
...
The release binary will be at `target/release/dure-sijang`.
...
Dure-Sijang supports building for multiple targets:
...
cd /opt/dure-sijang/mobile && ./build.sh
