//! Desktop entry point for Dure-Sijang
//!
...
//! - `--log [FILE]`: Enable file logging (defaults to dure-sijang.log if FILE not specified)

#![cfg_attr(target_os = "windows", windows_subsystem = "windows")]

use eframe::egui::{self, IconData};
use dure_sijang::dure_sijang_app::{self, DureSijangApp};

// GTK is required for wry WebView on Linux/OpenBSD
    ...
    impl InstructionWindow {
        fn new() -> Self {
            ...
            let instruction_text = "System does not have OpenGL 2.0+.\r\n\r\n\
                ...
                https://dure-sijang.github.io/docs/installation#download\r\n\r\n\
...
fn main() -> eframe::Result<()> {
        ...
        while i < args.len() {
            if args[i] == "--log" {
                ...
                if i + 1 < args.len() && !args[i + 1].starts_with("--") {
                    ...
                    log_file_opt = Some("dure-sijang.log".to_string());
                    i += 1;
                }
            } else {
                i += 1;
    ...
    if let Some(log_path) = &log_file {
        ...
        let log_level = if let Ok(config) = dure_sijang::Config::new() {
    ...
    if uninstall {
        ...
        use dure_sijang::install;
        use dure_sijang::install_stt::InstallResult;
    ...
    // Initialize GTK before creating webviews (required for wry on Linux/OpenBSD)
    #[cfg(all(not(target_arch = "wasm32"), any(target_os = "linux", target_os = "openbsd")))]
    {
