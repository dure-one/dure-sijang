...
const APP_NAME: &str = "dure-sijang";
const GITHUB_REPO: &str = "nikescar/dure-sijang";
const CURRENT_VERSION: &str = env!("CARGO_PKG_VERSION");

/// Get versioned app name (e.g., "dure-sijang-1.0.0")
                ...
                .and_then(|entries| {
                    entries
                        ...
                        .any(|entry| {
                            ...
                            let is_dure_sijang = name_str.starts_with(&format!("{}-", APP_NAME))
...
fn install_linux(paths: &InstallPaths, current_exe: &PathBuf) -> Result<String, String> {
    ...
    let icon_dest = paths.bin_dir.join("dure-sijang.png");
...
<dict>
    ...
    <string>app.dure.sijang</string>
    <key>CFBundleName</key>
    <string>Dure-Sijang {}</string>
    ...
    if let Some(ref key) = paths.uninstall_key {
        ...
        let reg_entries: Vec<(&str, &str, String)> = vec![
            ...
            (
                "URLInfoAbout",
                "REG_SZ",
                "https://dure-sijang.pages.dev".to_string(),
            ),
...
fn get_platform_download_url() -> Option<String> {
    ...
    let filename = match (target_os, target_arch) {
        // Windows x86_64: dure-sijang.exe
        ("windows", "x86_64") => "dure-sijang.exe",

        // macOS x86_64: dure-sijang-x86_64-apple-darwin.tar.gz
        ("macos", "x86_64") => "dure-sijang-x86_64-apple-darwin.tar.gz",

        // macOS aarch64: dure-sijang-aarch64-apple-darwin.tar.gz
        ("macos", "aarch64") => "dure-sijang-aarch64-apple-darwin.tar.gz",

        // Linux x86_64: dure-sijang-x86_64-unknown-linux-musl.tar.gz
        ("linux", "x86_64") => "dure-sijang-x86_64-unknown-linux-musl.tar.gz",

        // Linux aarch64: dure-sijang-aarch64-unknown-linux-musl.tar.gz
        ("linux", "aarch64") => "dure-sijang-aarch64-unknown-linux-musl.tar.gz",

        // Android: dure-sijang-all-signed.apk
        ("android", _) => "dure-sijang-all-signed.apk",
